...
const out = path.join(ROOT, process.env.NAVA_QA_OUT??'runs/debug/tutorial-final'); await mkdir(out, { recursive: true });
...
try {
  ...
  await waitFor(async () => (await h.api('/api/state')).body,s=>s.videoReady&&s.screensConnected===1,'renderer ready',30000);
  await h.command({action:'setLang',lang});
  if(process.argv.includes('--smoke-only')){
    await h.select('legacy-v3');await h.lockCrew(2);await exp('skip');await waitFor(async()=>(await h.api('/api/state')).body,s=>s.readiness?.ready,'legacy ready',30000);
    const smoke=spawn(process.env.ComSpec??'cmd.exe',['/d','/s','/c','npm run smoke:renderer'],{cwd:ROOT,windowsHide:true,stdio:'inherit',env:{...process.env,NAVA_CDP_PORT:String(port),NAVA_SERVER_PORT:new URL(h.base).port,NAVA_TEST_PIN:'9384'}});assert.equal(await new Promise(r=>smoke.once('exit',r)),0);
  }else if(process.argv.includes('--music-only')){
    ...
    await h.select('adults');await h.lockCrew(2);await exp('skip');await waitFor(async()=>(await h.api('/api/state')).body,s=>s.readiness?.ready,'adults ready',45000);await h.command({action:'start'});await h.command({action:'epilogue'});await h.command({action:'seek',time:57.5});await size(3840,2160);
  ...
  await h.select(PROFILES[0]);await h.lockCrew(2);await waitFor(async()=>(await h.api('/api/state')).body,s=>s.readiness?.ready,'profile audio ready',45000);
  await size(3840,2160);await exp('start');
  ...
  for(const profile of PROFILES){
    if(profile!==PROFILES[0]){await h.command({action:'restart'});await h.select(profile);await h.lockCrew(2);await exp('skip');await waitFor(async()=>(await h.api('/api/state')).body,s=>s.readiness?.ready,'profile ready',45000);await h.command({action:'start'});}
    ...
    await size(3840,2160);await new Promise(r=>setTimeout(r,350));await shot(`${profile}-finale-tv-3840`);let m=await expMetric();assert(!m.hidden&&!m.overflow);assert(m.text.includes({ro:'1 simbol primit',en:'1 symbol received',fr:'1 symbole reçu'}[lang]));assert.equal(await evaluate(`document.querySelectorAll('.experience-seat[data-state="observing"]').length`),1);records.push({profile,finale:m});
