...
export function redactLog(value:unknown,depth=0,seen=new WeakSet<object>()):unknown{
 ...
 if(typeof value==='string')return value.slice(0,16000).replace(/\bsk_[a-zA-Z0-9_-]+/g,'[secret]').replace(/Bearer\s+[\w.+/=-]+/gi,'Bearer [secret]').replace(/((?:api[_-]?key|token|password|credential|otp|recoveryCodes?|mfa|secret|pin|authorization|cookie)["']?\s*[:=]\s*["']?)[^\s,"'&}]+/gi,'$1[secret]').replace(/([?&](?:code|state)=)[^&\s]+/gi,'$1[secret]').replace(/(\/souvenir\/[^/\s]+\/)[a-f0-9]{64}/g,'$1[secret]').replace(/data:(?:image|audio)\/[^\s]+/g,'[media omitted]');
 ...
 return Object.fromEntries(Object.entries(value).slice(0,100).map(([key,v])=>[key,/(token|password|credential|otp|recoveryCodes?|mfa|secret|api.?key|authorization|cookie|dataurl|audio_base64|^pin$|pinhash)/i.test(key)?'[redacted]':redactLog(v,depth+1,seen)]));
}
