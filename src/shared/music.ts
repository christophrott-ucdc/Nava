...
export interface MusicMix {duckDb:number;duckAttackSec:number;duckReleaseSec:number;silence:{phase:Phase;startSec:number;endSec:number}}
export interface MusicManifest extends MusicMix {version:1;tracks:MusicTrack[];scenarios?:Record<string,MusicMix & {tracks:MusicTrack[]}>}
export function musicMix(manifest:MusicManifest,show:ShowFile):MusicMix{return manifest.scenarios?.[show.scenario?.id??'']??manifest;}
export function musicCues(manifest:MusicManifest,show:ShowFile):AmbientCue[]{
  return (manifest.scenarios?.[show.scenario?.id??'']?.tracks??manifest.tracks).flatMap(t=>{
...
export function musicSilenceGain(phase:Phase|null,time:number,silence:MusicMix['silence']={phase:'play',startSec:232,endSec:246}):number{return phase===silence.phase&&time>=silence.startSec-.5&&time<silence.endSec?Math.max(0,Math.min(1,(silence.startSec-time)/.5)):1;}
