...
export function createMissionOverlay(parent:HTMLElement){
  ...
  return {element:box,update(s:MissionSnapshot){
    ...
    detail.textContent=s.suspended?'Misiunea este în pauză. Operatorul pregătește continuarea.':final?s.summary.lines.slice(0,2).join(' · '):s.stage?`Cinci posturi, zece perspective · Etapa ${s.stage} din 3`:'Călătorim împreună. Următoarea descoperire ne așteaptă.';
    object.hidden=!s.lantern||!final;
    if(s.lantern&&final){
      const colors=['#ffb86b','#ffe38f','#90dbff','#76e0bc','#b9a3ff'];
      const pieces=s.lantern.map((p,i)=>{const a=i*Math.PI/5-Math.PI/2,x=160+98*Math.cos(a),y=135+90*Math.sin(a);return `<path d="M160 135 L${x} ${y}" stroke="${p.linked?'#ffe38f':'#ffffff35'}" stroke-width="${p.linked?5:2}" ${p.linked?'':'stroke-dasharray="4 5"'}/><circle cx="${x}" cy="${y}" r="23" fill="${p.mounted?colors[Math.floor(i/2)]:'#ffffff12'}" stroke="${p.found?colors[Math.floor(i/2)]:'#ffffff66'}" stroke-width="3"/><text x="${x}" y="${y+5}" text-anchor="middle" fill="${p.mounted?'#172337':'#fff'}" font-size="14">${Math.floor(i/2)+1}${i%2?'B':'A'}</text>`;}).join('');
