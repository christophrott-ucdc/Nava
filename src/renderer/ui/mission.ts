...
export function createMissionOverlay(parent:HTMLElement){
  ...
  return {element:box,update(s:MissionSnapshot){
    ...
    box.hidden=!!s.experience?.tvOnly||s.scenarioId==='legacy-v3'||s.state.state==='idle'||!!s.experience?.active||!!s.experience?.finaleActive||(!final&&!s.stage);
    ...
    if(s.lantern&&final){
      ...
      object.innerHTML=`<svg viewBox="0 0 320 270" role="img" aria-label="Steaua echipajului: doar piesele și legăturile confirmate sunt colorate">${pieces}<path d="M160 87 L173 117 L206 121 L181 143 L188 176 L160 159 L132 176 L139 143 L114 121 L147 117 Z" fill="#ffe9a9" stroke="#fff3d2" stroke-width="3"/><path d="M160 103 L165 128 L189 132 L167 141 L160 163 L153 141 L131 132 L155 128Z" fill="#ffb96c"/></svg>`;
      const mounted=s.lantern.filter(p=>p.mounted).length,linked=s.lantern.filter(p=>p.linked).length;
      const caption=document.createElement('p');caption.textContent=`${mounted} ${mounted===1?'piesă așezată':'piese așezate'} · ${linked} ${linked===1?'legătură aprinsă':'legături aprinse'}`;object.append(caption);
    }
