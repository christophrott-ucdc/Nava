...
export function createMissionOverlay(parent:HTMLElement){
  ...
  return {element:box,update(s:MissionSnapshot){
    ...
    if(s.lantern&&final){
      ...
      const caption=document.createElement('p');caption.textContent=`${mounted} ${mounted===1?'piesă așezată':'piese așezate'} · ${linked} ${linked===1?'circuit închis':'circuite închise'}`;object.append(caption);
    }
