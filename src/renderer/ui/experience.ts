...
export function createExperienceOverlay(parent:HTMLElement,options:{audio:boolean;visual:boolean;baseUrl:string;volume:number;outputDeviceId?:string;clockOffset:()=>number;onNarration:(instance:string,status:'ended'|'error')=>void;onAudioActive?:(active:boolean)=>void}){
  ...
  let language:Lang='ro',loading=false,manifestGeneration=0;
  async function loadManifest(){
    if(loading||disposed)return;loading=true;const mine=manifestGeneration,wanted=language;
    try{const r=await fetch(new URL(`/api/experience/voices?lang=${wanted}`,options.baseUrl),{cache:'no-store'});if(r.ok){const value=await r.json() as NarratorManifest;if(!disposed&&mine===manifestGeneration&&(value.lang??'ro')===wanted)manifest=value;}}
    catch{}finally{if(mine===manifestGeneration)loading=false;}
    if(!disposed&&mine===manifestGeneration){if(last&&manifest)update(last);else if(!manifest)setTimeout(()=>{if(mine===manifestGeneration)void loadManifest();},3000);}
  }
  ...
  function syncAudio(s:MissionSnapshot){
    ...
    if(id!==instance){
      stop();instance=id;const mine=generation;audio.src=new URL(`/assets/experience/voice/${language}/${clip.file}`,options.baseUrl).href;
  ...
  function update(s:MissionSnapshot){
    if(s.state.lang!==language){language=s.state.lang;manifestGeneration++;loading=false;manifest=null;completed='';key='';stop();}
    last=s;if(!manifest)void loadManifest();syncAudio(s);const e=s.experience,final=!!e?.finaleActive;
    ...
    if(final&&s.scenarioId==='age-5-10')crew.clear();
    else crew.update(relay,[{element:world}],{reduced:s.accessibility.reducedMotion||s.accessibility.reducedStimuli,paused:s.suspended||!!e?.paused,flat:s.accessibility.showVisualGuidance===false||s.accessibility.reducedStimuli});
    ...
    const step=e!.step;
    title.textContent=final?(s.scenarioId==='age-5-10'?'Steaua noastră poartă ceva de la fiecare':'Nava poartă ceva de la fiecare'):step==='touch'?'Călătoria începe cu tine':step==='practice'?EXPERIENCE_PRACTICE[s.scenarioId].title:step==='cooperate'?'Aprindem nava împreună':'Acum suntem un echipaj';
    ...
    if(s.scenarioId==='age-5-10'&&copy.textContent)copy.textContent=copy.textContent.replace('Căpitanul','Comandantul');
    if(final)copy.textContent=s.scenarioId==='age-5-10'?'Alege un simbol și trimite-l de pe tabletă. Va apărea lângă personajul tău și va aprinde o lumină în jurul stelei.':'Alege un simbol și trimite-l de pe tabletă. Va apărea lângă personajul tău și pe navă.';
  ...
  return{element:box,update,setVolume(value:number){audio.volume=Math.max(0,Math.min(1,value));},dispose(){disposed=true;stop();crew.dispose();window.removeEventListener('pointerdown',unlock);window.removeEventListener('keydown',unlock);box.remove();}};
}
