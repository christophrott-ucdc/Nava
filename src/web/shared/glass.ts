...
export function createTabletAudio(){let unlocked=false,enabled=true;const pool=new Map<TabletSound,HTMLAudioElement>();for(const name of ['tap','pick','confirm','start','thanks'] as TabletSound[]){const audio=new Audio(`/tablet/sfx/${name}.mp3`);audio.preload='auto';audio.volume=.35;pool.set(name,audio)}
 ...
 return {setVolume(value:number){const gain=Number.isFinite(value)?Math.max(0,Math.min(1,value)):0;for(const a of pool.values())a.volume=.35*gain},setEnabled(value:boolean){enabled=value;if(!value)for(const a of pool.values()){a.pause();a.currentTime=0}},play(name:TabletSound){if(!enabled||!unlocked)return;const a=pool.get(name)!;a.currentTime=0;void a.play().catch(()=>{})}};
}
