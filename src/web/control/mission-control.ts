...
export function createMissionControl(deps:{snapshot():{state:ShowState|null;role:string|null};dispatch(command:Command):Promise<void>}):void{
  ...
  async function loadDiagnostics():Promise<void>{
    ...
    if(report.kind==='rehearsal'){
      ...
      if(report.status==='running'){const progress=document.createElement('progress');if(report.expectedDurationSec){progress.max=report.expectedDurationSec;progress.value=Math.min(report.expectedDurationSec,report.elapsedSec);}progress.setAttribute('aria-label','Timpul repetiției conform scenariului activ');box.append(progress);}
