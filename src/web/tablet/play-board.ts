...
export function createPlayPanel(zone: 'A' | 'B', send: (value: string) => void) {
  ...
  const flow=el('div','play-discovery-flow'),takeaway=el('p','play-discovery-copy');takeaway.setAttribute('role','status');insight.append(achievement,flow,takeaway);
  ...
  foot.append(feedback, observe); head.append(tools); panel.append(head, instruction, host, insight, foot);
