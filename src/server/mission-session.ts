...
export class MissionSession {
  ...
  snapshot(state:ShowState,post?:TabletPost):MissionSnapshot {
    const r=this.record,stage=this.stage(state),progress=this.progress();
    const view=post&&stage?scenarioView(progress,stage,post):null;
  ...
  accept(event:MissionEvent,post:TabletPost,state:ShowState):{ok:boolean;status:string;eventId:string;reason?:string} {
    ...
    if(this.progress().participants&&!this.progress().participants!.includes(`${post}${event.zone}`))return result('inactive-seat');
    ...
    const changed=applyScenarioAction(this.progress(),{stage,post,zone:event.zone,action:'choose',value:event.value});
  ...
  conditions():Set<string>{return scenarioConditions(this.progress());}
